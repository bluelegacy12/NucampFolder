...
SELECT * FROM products WHERE discontinued; -- discontinued is a boolean value and in this scenario is equivalent to  ' = true'
...
SELECT first_name, last_name FROM employees WHERE reports_to IS NULL ORDER BY employee_id;
...
UNION SELECT company_name, address, city, region, postal_code, country FROM customers
...
SELECT c.company_name, SUM(od.quantity) FROM order_details od
JOIN orders o ON o.order_id = od.order_id
JOIN customers c ON c.customer_id = o.customer_id
GROUP BY c.customer_id HAVING SUM(od.quantity) >= 500
